...
Now let me build this. This is a substantial JavaFX 3D application — I'll write the complete single-file implementation:Here's the complete prototype with a deep breakdown of every architectural decision.

---

## What was built

A single `WorkspaceGraph3D.java` (~400 lines) with zero external dependencies beyond JavaFX itself. Here's the full technical breakdown:

---

## How to Run

**Prerequisites:** Java 17+ and JavaFX 17+ (download the SDK from [gluonhq.com/products/javafx](https://gluonhq.com/products/javafx/))

**Quickest path — Maven (recommended):**
Create a `pom.xml` with this plugin config and run `mvn javafx:run`. The comment block at the top of the file has the exact snippet to paste.

**Direct compile path (Linux/Mac):**
```bash
...
```
**Windows:** same but use `set FX=...` and `%FX%`.

---

## 3D Math vs. 2D — The Core Concept

| Concern | 2D Canvas / Pane | 3D SubScene + PerspectiveCamera |
|---|---|---|
| **Axes** | X→right, Y↓down | X→right, Y↓down, **Z↗toward you** |
| **Origin** | Top-left corner | World centre (0, 0, 0) |
| **Depth** | Faked with draw-order | Real perspective: far = smaller |
| **Camera** | Implicit — you see everything | Explicit `PerspectiveCamera` you position and rotate |
| **Lighting** | Irrelevant | `AmbientLight` + `PointLight` affect `PhongMaterial` |
| **Rotation** | `canvas.rotate(angle)` | `Rotate(angle, axisVector)` transform on a `Group` |

The trickiest part: a `Cylinder` in JavaFX always starts aligned to the **Y-axis**. To draw an edge from node A to node B, we need to:
1. Place the cylinder at the **midpoint** of A and B.
2. Set its **height** = distance(A, B).
3. Rotate it by computing the **cross product** of the default Y-axis with the A→B direction vector (giving the rotation axis), and the **dot product** for the angle. That's the `GraphEdge.update()` method.

---

## Physics Architecture

The `AnimationTimer` runs at ~60 fps and does three physics passes per tick:

**1. Repulsion (Coulomb's Law)** — Every pair of nodes checks `O(n²)` and pushes each other away with force `k / distance²`. Works in full X, Y, Z.

**2. Attraction (Hooke's Law)** — Each edge acts as a spring pulling its two endpoints toward each other. If distance > `SPRING_LENGTH` they attract; if closer they repel.

**3. Euler Integration** — `velocity += force`, `position += velocity`. Damping (0.85) bleeds off velocity each frame — acts as air resistance that makes the system settle.

---

## What to Extend First

These are the natural next steps, roughly in order of hackathon impact:

**1. Real window data** — Replace `MOCK_WINDOWS` with a JNA/ProcessHandle call to enumerate actual running OS windows. On Linux: parse `/proc/[pid]/comm`. On Windows: use `EnumWindows` via JNA.

**2. Click-to-focus interaction** — Add `node.sphere.setOnMouseClicked(e -> focusWindow(node))` and call the OS focus API.

**3. Edge weights from shared context** — If VS Code and Terminal were used together, increase the edge spring constant between them so they cluster closer.

**4. Label billboarding** — Instead of the 2D overlay trick, put `Text` in a `Billboard` Group that always counter-rotates against the camera so labels always face you.

**5. Upgrade O(n²) → Barnes-Hut** — For 50+ nodes the repulsion loop gets slow. Implement an octree (the 3D version of a quadtree) to approximate far-away node clusters as a single mass, dropping cost to `O(n log n)`.
### CODE ###
