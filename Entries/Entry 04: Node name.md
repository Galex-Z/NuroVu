Aug 7, 2026
beta5
# Added the ability to specify custom node names prior to spawning them in the 3D viewport.

Key Changes

Dynamic Node Naming: Introduced a text input field in the sidebar allowing custom node labels prior to creation.

Auto-Fallback & Formatting: Empty inputs (or inputs with only whitespace) automatically fall back to 1-based default labels (e.g., Node 1, Node 2). Leading/trailing whitespace is automatically trimmed.

Streamlined UX Workflow: Bound the Enter key on the input field directly to the spawn action, enabling rapid node creation without needing to switch between keyboard and mouse. Input clears automatically upon successful node creation.

UI Integration: Updated the sidebar layout with matching panel styling and placeholder prompt text.
Claude's response
Minimal change — just the sidebar and spawnNewNode. Let me read those sections first.

Viewed 4 files
Viewed 4 files

Three edits: add the field, wire the TextField into the sidebar, update spawnNewNode.

Edited 4 files, read a file
Edited 4 files, read a file

Five edits, nothing else touched. Here's what each one does and why it was done that way.

What changed

1. nameField as an instance field

java
private javafx.scene.control.TextField nameField;

It needs to be readable by spawnNewNode(), which is a separate method from buildSidebar(). Keeping it as a field is the standard JavaFX pattern for UI elements that are written in one method and read in another — same reason statusLabel and linkBtn are fields.

2. The TextField in the sidebar

java
nameField.setPromptText("e.g. Notepad");
nameField.setOnAction(e -> spawnNewNode());

setPromptText() shows greyed-out placeholder text when the field is empty — it never counts as actual input. setOnAction() fires when the user presses Enter inside the field, wiring it to the exact same method as the button so both interaction paths behave identically. The dark styling matches the rest of the panel.

3. spawnNewNode() updated

java
String typed = nameField.getText().trim();
String name  = typed.isEmpty() ? "Node " + (spawnIndex + 1) : typed;

trim() strips accidental leading/trailing spaces so a field containing only spaces is treated the same as empty. The fallback uses spawnIndex + 1 (one-based) rather than spawnIndex so the first auto-named node is "Node 1" rather than "Node 0", which reads more naturally.

java
nameField.clear();

This runs after the node is successfully added — if addNode() were to throw for some reason, the field wouldn't be silently wiped before the user could see their input.

4. Enter key = button

Because nameField.setOnAction and spawnBtn.setOnAction both call spawnNewNode(), the user can type a name and hit Enter without ever touching the mouse — useful for quickly building out a graph during a hackathon demo.
#CODE#
