...
Multi-Window Discovery API: Refactored native window search from findWindow() (which returned on first match) to findAllWindows(), which enumerates all visible top-level windows and returns a collection of WindowEntry records holding both window handles (HWND) and original-case title strings.

Smart 3-Way Resolution Flow:

0 Matches: Spawns the node in an unbound state.

1 Match: Automatically binds directly to the target window (preserving single-match workflow efficiency).

2+ Matches: Prompts the user with an interactive selection dialog to resolve ambiguity explicitly.

Disambiguation Picker Modal (showWindowPicker): Implemented an APPLICATION_MODAL JavaFX dialog displaying full window titles (e.g., distinguishing between distinct browser tabs sharing common keywords). Includes an option to leave the node unbound.

Thread Safety & String Normalization: Preserved off-thread window enumeration via background scanning, ensuring UI responsiveness during native calls. Title comparisons are normalized to lower-case while preserving original casing for UI presentation.
Cloude's response:
Good observation. Right now findWindow stops at the first title match — so if you have three Chrome tabs open, you always get whichever one EnumWindows happens to enumerate first.

The fix: when multiple windows match the search string, don't auto-bind — instead show a picker dialog listing all matches with their full titles, and let the user click the exact one they want.

Viewed 2 files
Viewed 2 files

Good. Three edits needed:

Replace findWindow (returns first match) with findAllWindows (returns every match as a list)
Add a WindowEntry record to hold (HWND, fullTitle) pairs
Update spawnNewNode — if 1 match: auto-bind as before; if 2+: show a JavaFX picker dialog
Replace findWindow with findAllWindows returning a list, add WindowEntry record
Replace findWindow with findAllWindows returning a list, add WindowEntry record

Now rewrite spawnNewNode to use the new list and show a picker when needed:
...
What changed
findAllWindows replaces findWindow

The old function returned on the first match (return false to stop EnumWindows). The new one removes that early exit and always returns true from the callback, collecting every matching window into a List<WindowEntry>. The WindowEntry record carries both the HWND and the full title string — the full title is what we show in the picker so the user can tell windows apart.

One subtle bug fixed along the way: the old code did title.toLowerCase().contains(lower) but stored the lowercased title. The new code stores the original-case title in the record (for display) and only lowercases for the comparison:

java
String title = new String(buf, 0, len);           // full case, for display
if (title.toLowerCase().contains(lower)) { ... }  // lowercase, for matching
spawnNewNode — three-way resolution
0 matches → node stays unbound, no ⚡ badge
1 match   → auto-bind silently (same experience as before for unambiguous names)
2+ matches → showWindowPicker() opens a modal dialog

The scanner thread still runs off the FX thread (so EnumWindows never blocks the render loop), and Platform.runLater still marshals back for the UI mutation — that architecture didn't change.

showWindowPicker — the disambiguation dialog

It's a plain Stage set to APPLICATION_MODAL, containing one Button per match. Each button shows the full window title — so "YouTube - Google Chrome", "Gmail - Google Chrome", and "GitHub - Google Chrome" are clearly distinguishable even though all three matched the query "chrome". Clicking a button calls bindNode() and closes the dialog. "Leave unbound" closes without binding, which is always safe.

The width is capped at 480px but height is sizeToScene() so it auto-fits regardless of how many matches there are.
#CODE#
