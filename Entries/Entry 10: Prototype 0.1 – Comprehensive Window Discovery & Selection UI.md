...
Unfiltered Window Enumeration (listAllWindows): Updated the Win32 window scanner to collect all active, visible top-level windows without pre-filtering during the native EnumWindows callback. Decoupled OS-level enumeration from UI-level filtering logic.

Streamlined Creation Workflow: Updated node creation to trigger the window picker dialog consistently, replacing automated fuzzy-matching heuristics with explicit user selection.

Interactive Disambiguation Dialog (showWindowPicker):

Live Search & Filtering: Integrated a real-time TextField filter with a listener that updates visible window options dynamically as the user types, accompanied by a dynamic results counter (e.g., "N of M windows shown").

Scrollable Layout: Wrapped window option rows in a fixed-height ScrollPane (fitToWidth=true) to handle large numbers of open windows gracefully without vertical clipping.

Keyboard Focus Navigation: Automatically assigns focus to the search field upon modal launch (requestFocus), allowing instant typing without requiring a mouse click.

Explicit Unbound Fallback: Added a persistent "Don't bind to any window" action, ensuring nodes can remain unbound at the user's discretion.

Asynchronous Threading: Maintained off-thread native window scanning to preserve smooth 60 FPS rendering, marshaling UI dialog construction back to the JavaFX Application Thread.
Claude's response:
Clean approach — scan first, pick second, every time. One rewrite of spawnNewNode and showWindowPicker, plus a small tweak to buildSidebar to remove the now-misleading status hint about auto-scanning.

Read spawnNewNode, bindNode, showWindowPicker
Read spawnNewNode, bindNode, showWindowPicker

Good. Also need to check findAllWindows — the new flow enumerates everything, no search filter at all.

Read findAllWindows to understand its current signature
Read findAllWindows to understand its current signature

Now I have everything. Three edits:

Add listAllWindows() — same as findAllWindows but no filter at all
Rewrite spawnNewNode — scan everything, always show picker
Rewrite showWindowPicker — scrollable list, search box to narrow it down live, counts in the header
Edited 2 files
Edited 2 files

Now rewrite showWindowPicker with a live search box and scrollable list:

Edited a file, read a file
Edited a file, read a file

Here's what changed and every decision behind it.

What changed
listAllWindows() — no filter at scan time

The old findAllWindows(name) filtered during the EnumWindows callback. The new listAllWindows() collects every visible window with a non-empty title unconditionally. Filtering is now the picker's job, not the scanner's. This is the right separation: the OS tells you what exists, the UI decides what to show.

findAllWindows is kept but reimplemented as a thin filter over listAllWindows(), in case you want it for something else later.

spawnNewNode — always opens the picker

The three-way if/else (0 matches → nothing, 1 → auto-bind, 2+ → picker) is gone. It's now unconditional:
...
The scan still runs off-thread so EnumWindows never blocks a render frame.

showWindowPicker — the main upgrade

Live search box — a TextField with a textProperty listener that calls rebuildRows on every keystroke. The full all list is never mutated; filtering creates a new view via stream().filter().toList() each time. The subheader updates to show "N of M windows shown" as you type, so you always know how many you've narrowed it down to.

Scrollable list — the rows live inside a VBox wrapped in a ScrollPane with setFitToWidth(true) and VBox.setVgrow(scroll, ALWAYS), so the dialog has a fixed 460px height and scrolls when there are more windows than fit. Without this, a user with 40 open windows would get a dialog taller than their screen.

"Don't bind to any window" sits below the scroll area, always visible regardless of scroll position. Closing the dialog via the X button does the same thing — the node was already spawned before the picker opened, so closing without picking just leaves it unbound.

Search box gets focus automatically via Platform.runLater(search::requestFocus) — the user can start typing the moment the dialog appears without clicking into it first.
#CODE#
