...
Root Cause Analysis: Identified that invoking ShowWindow(SW_RESTORE) on non-minimized or already visible windows was triggering the Win32 API's pre-maximized restoration path, forcing windows to collapse or shrink back to un-maximized internal boundaries.

State-Aware Activation Logic:

Added IsIconic(HWND) checks to distinguish genuinely minimized (iconified) windows from visible ones.

Minimized Windows: Trigger SW_RESTORE to bring the application out of the taskbar, followed by targeted dimension restoration.

Visible / Maximized Windows: Switched to SW_SHOW, bringing the window to the foreground while leaving its current position, layout, and bounds completely untouched.

Native Dimension Pinning:

Integrated GetWindowRect(HWND, RECT) to take an exact pixel-accurate snapshot of window dimensions prior to activation.

Mapped SetWindowPos with the SWP_NOMOVE flag to re-apply exact height and width constraints after restoring minimized windows without shifting their screen coordinates.
Claude's response:
The fix is to call GetWindowPlacement before focusing to capture the window's current size and position, then restore it with SetWindowPlacement after ShowWindow and SetForegroundWindow.

Read the User32Extended interface and focusWindow method
Read the User32Extended interface and focusWindow method

Good. The problem is SW_RESTORE — it tells Windows to restore a minimised window to its last known size, but for a window that's already visible (not minimised) Windows interprets it as "restore from maximised", which collapses it. The fix is to check the window's current state first with GetWindowPlacement, then use SW_SHOW (which never resizes) for already-visible windows and only use SW_RESTORE for genuinely minimised ones. We also need the WINDOWPLACEMENT structure and GetWindowRect/SetWindowPos to pin the dimensions afterward.

Edited a file, read a file
Edited a file, read a file

One method rewritten, three new Win32 functions mapped. Here's exactly why the resize was happening and what each piece of the fix does.

Root cause — SW_RESTORE is the wrong flag for visible windows

SW_RESTORE (value 9) has a specific meaning in Win32:

If the window is minimised → restore it to its pre-minimise size and position ✓
If the window is maximised → restore it to its pre-maximise (normal) size ← this is the shrink you were seeing
If the window is already normal → Windows still runs the restore path, which can snap it to whatever "restored rect" is stored internally — not necessarily the current size

So calling SW_RESTORE on a Chrome window that's already at a comfortable size was telling Windows "shrink this back to its pre-maximise dimensions", which it dutifully did.

The fix — three new Win32 calls

IsIconic(HWND) — returns true only if the window is genuinely minimised (iconified to the taskbar). This lets us branch:

minimised  → must use SW_RESTORE (only way to un-minimise), then pin size back
visible    → use SW_SHOW instead, which never touches size or position

GetWindowRect(HWND, int[4]) — fills a four-element array with {left, top, right, bottom} in screen pixels before we call anything else. Width = right - left, height = bottom - top. This is our snapshot.

SetWindowPos(HWND, ..., width, height, flags) — called only after SW_RESTORE on a minimised window, to pin the dimensions back to the snapshot. SWP_NOMOVE tells Windows to ignore the X/Y parameters and only apply the size, so the window lands where the user expects it, not at the origin.

The result for the two cases:

Window state	Path taken	Size change?
Minimised	SW_RESTORE → SetWindowPos to pin size	No
Visible / maximised	SW_SHOW only	No
#CODE#
