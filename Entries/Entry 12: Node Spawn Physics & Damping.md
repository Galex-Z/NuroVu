...
Good, I have everything. Two small edits: add spawnDamping to GraphNode, then use it per-node in the integrate step.
Let me restore the last clean version before the visual changes — that means before the starfield, aura canvas, upgraded lighting, and Fibonacci lattice were added.
The file is very long. Let me write the clean restored version directly:

Ran a command, read a file
Ran a command, read a file

Clean slate — all visual changes gone, spawn damping added. Here's what's in this file:

Removed (visual polish experiment): starfield, aura canvas, Fibonacci lattice, upgraded lighting, upgraded node/edge materials, star twinkle loop.

Kept (all functional features): JNA window binding, window picker, focus/restore on double-click, link mode, delete, zoom, pan, billboard labels, named spawn.

Added (the fix you asked for): spawn damping. Two fields on every GraphNode:

java
double spawnDamping = 0.98;  // starts near-frozen
int    spawnTick    = 0;     // counts frames since spawn

In the integrate step, instead of applying the global DAMPING (0.85) to every node equally, each node now uses its own spawnDamping:

frame 0:   spawnDamping = 0.98  → velocity bleeds off 98% per tick, barely moves
frame 60:  spawnDamping = 0.915 → halfway relaxed
frame 120: spawnDamping = 0.85  → fully settled, behaves like every other node

The linear interpolation over 120 frames (~2 seconds at 60fps) means the node drifts gently into its natural position rather than being flung across the scene the moment the forces hit it.
#CODE#
...
public class WorkspaceGraph3D extends Application {
    ...
    private void attachMouseHandlers(StackPane viewport) {
        ...
        viewport.setOnMouseClicked(e -> {
            ...
            if (linkModeActive) {
                if (linkSource != null) { clearLinkSelection(); setStatus("Deselected.\nClick a node to\nselect it first."); }
            } else {
                clearSelection();
            }
        });
    ...
    private void syncSceneGraph() {
        ...
        for (GraphNode nd : nodes) { nd.applyPosition(); nd.updateBillboard(ay, ax); }
