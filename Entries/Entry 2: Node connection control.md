...
buildInitialEdges() is gone entirely. The startup loop now just calls addNode() three times with no addEdge() calls. The repulsion physics still fires for disconnected nodes, so they naturally drift apart and float in space independently.

2. Disconnected spawn

spawnNewNode() was stripped down to a single addNode() call. The old lines that wired the new node to rng.nextInt(nodes.size()-1) and nodes.get(nodes.size()-2) are deleted. Spawned nodes float freely until the user links them.

3. Link Mode — the state machine

The heart of the new feature is a two-state machine tracked by two fields:

linkModeActive  →  bool: is the mode on?
linkSource      →  GraphNode or null: has the user clicked the first node?

The state transitions:

Current state	User action	Result
Mode OFF	Click toggle	Mode ON, linkSource = null
Mode ON, no selection	Click sphere A	linkSource = A, sphere glows
Mode ON, A selected	Click sphere A again	Deselect, linkSource = null
Mode ON, A selected	Click sphere B	Link or unlink A↔B, reset
Mode ON, any	Click empty space	Deselect, linkSource = null
Mode ON, any	Click toggle	Mode OFF, clear selection

Edge toggling — findEdge(a, b) scans the edge list for a cylinder that connects the same two nodes in either direction. If found, removeEdge() pulls it from both edges and world3D.getChildren(). If not found, addEdge() creates one. This means clicking a pair a second time removes their connection — which is both intuitive and useful for the hackathon demo.

4. The event.consume() problem (critical detail)

When the user clicks a sphere in link mode, two handlers would normally fire:

The sphere's setOnMouseClicked → selects the node
The viewport's setOnMouseClicked → immediately deselects it (because linkSource != null)

Without e.consume() on the sphere handler, you'd never be able to select anything — the selection would be created and destroyed in the same event cycle. consume() stops the event from bubbling up to the parent, so only the sphere handler fires.

5. Camera drag disabled in Link Mode
java
viewport.setOnMouseDragged(e -> {
    if (linkModeActive) return;   // ← this line
    ...
});

Without this guard, any mouse movement while trying to click a sphere would spin the scene, making targeting nodes frustrating. In Link Mode the camera is frozen; you can still rotate by toggling back to Navigate Mode first.

6. Visual feedback — the highlight glow

When a node is selected as the first link target, setHighlight(true) interpolates its diffuse color 55% toward white. This uses PhongMaterial.setDiffuseColor() rather than emissive maps (which require a texture image) — a clean prototype-friendly approach. The PhongMaterial reference is kept as a field on GraphNode precisely so it can be mutated mid-run.
# THE CODE#
