Aug 7, 2026
Beta 4
# Added camera zoom-out capabilities so nodes remain in view when moving toward viewport edges.
Promoted camera reference to class scope for handler access.
Added scroll-driven Z-axis translation with input normalization for trackpads and scroll wheel.
Enforced distance boundaries (clamped $Z$-axis) to prevent geometry clipping.
Enabled zoom functionality across both Navigation and Link modes.
Updated viewport UI control hints.
Claude's response
The quickest correct approach in JavaFX 3D is to move the camera along its Z axis on scroll — camera.setTranslateZ() already exists, so it's a one-handler addition.

Viewed 3 files
Viewed 3 files

Good. Three targeted edits needed: promote camera to a field, add zoom constants, add the scroll handler.

Edited 4 files
Edited 4 files

Now update the hint in the sidebar so users know about the scroll gesture:

Edited a file, read a file
Edited a file, read a file

Done — four surgical edits, nothing else touched. Here's what changed and why each decision was made the way it was.

What was changed

1. camera promoted to an instance field

The original camera was a local variable inside buildUI(), so it was unreachable from anywhere else. Promoting it to a field (private final PerspectiveCamera camera) lets the scroll handler (which is attached later on the viewport) close over it naturally.

2. Three zoom constants added

java
private static final double ZOOM_SPEED = 20.0;   // world-units per scroll tick
private static final double ZOOM_MIN   = 100.0;  // closest the camera can get
private static final double ZOOM_MAX   = 1800.0; // furthest it can go

These are tunable independently without touching the handler logic.

3. The scroll handler
...
The key coordinate-system detail: the camera starts at Z = -550 (negative = behind the world, looking forward). Scrolling up gives a positive deltaY, which increases Z (less negative = moves toward the scene = zoom in). Scrolling down gives a negative deltaY, pulling the camera further back = zoom out. The clamp keeps Z between -1800 and -100 so you can never clip through the geometry or lose the scene entirely.

The /40.0 divisor normalises deltaY because JavaFX reports scroll deltas in pixels, which vary wildly between a trackpad (tiny deltas, many events) and a mouse wheel (large deltas, few events). Dividing by 40 gives roughly 1 world-unit per "notch" on both devices.

4. Works in both modes — the scroll handler is on the viewport directly and doesn't touch linkModeActive, so zooming works whether you're rotating the graph or linking nodes.
#CODE#
