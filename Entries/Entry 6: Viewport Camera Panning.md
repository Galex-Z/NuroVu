...
Coordinate-Space-Aware Translation: Applied spatial translations directly to the root world3D Group rather than shifting camera coordinates. This ensures panning tracks cursor movement correctly relative to screen space regardless of current rotation angles.

Link Mode Ergonomics: Allowed viewport panning while Link Mode is active to help users reach distant nodes without breaking workflow state.

Event Scoping: Restricted background deselection clicks strictly to MouseButton.PRIMARY, preventing accidental deselection during right-click pan gestures.
Claude's response:
What changed and why
Splitting the drag handler

The old setOnMouseDragged had one path (rotate). The new one branches on which button is held using JavaFX's built-in query methods:

java
if (e.isPrimaryButtonDown())    { /* left  → rotate */ }
else if (e.isSecondaryButtonDown()) { /* right → pan    */ }

setOnMousePressed still records mouseX/mouseY for both buttons with a single handler since both drag paths need the same delta calculation — no duplication needed.

Why translate world3D instead of moving camera.setTranslateX/Y

This is the key design decision. There are two naive approaches that both break:

Broken option A — move camera X/Y:

java
camera.setTranslateX(camera.getTranslateX() + dx);

The camera's axes are fixed to the screen, ignoring the world rotation. After rotating the scene 90° around Y, the world's "right" is now the screen's "into-the-screen". A horizontal drag would slide the camera sideways in screen space but the world would appear to move forward — completely wrong feel.

Broken option B — decompose into world axes:
You'd need to unproject the screen delta through the inverse of the combined rotateX × rotateY matrix. Doable but 15+ lines of trigonometry for no real gain.

Correct approach — translate world3D:

java
world3D.setTranslateX(world3D.getTranslateX() + dx * PAN_SPEED);
world3D.setTranslateY(world3D.getTranslateY() + dy * PAN_SPEED);

JavaFX applies transforms in list order: rotateX and rotateY are already on world3D, and setTranslate* applies after those rotations in the scene graph. So moving world3D by +dx in screen space correctly moves the already-rotated world — which is exactly the Blender-style pan feel where the geometry tracks your cursor regardless of the current viewing angle.

Link mode safety

The right-drag pan path has no linkModeActive guard — panning while in link mode is harmless and actually useful (you might need to pan to reach a node you want to click). Only left-drag rotation is blocked in link mode, exactly as before.

setOnMouseClicked now checks PRIMARY button

Previously an accidental right-click on empty space would have triggered the link deselect. Adding the MouseButton.PRIMARY check means only left-clicks deselect, keeping right-click strictly for panning.
#CODE#
...
public class WorkspaceGraph3D extends Application {
    ...
    private void attachMouseHandlers(StackPane viewport) {
        ...
        // ── Left-click on empty space in link mode → deselect ────────
        viewport.setOnMouseClicked(e -> {
            if (e.getButton() == javafx.scene.input.MouseButton.PRIMARY
                    && linkModeActive && linkSource != null) {
                clearLinkSelection();
                setStatus("Deselected.\nClick a node to\nselect it first.");
            }
        });
