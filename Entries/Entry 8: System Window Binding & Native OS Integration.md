...
Win32 Native Interop: Integrated JNA (User32Extended mapping EnumWindows, GetWindowTextW, IsWindowVisible, SetForegroundWindow, and ShowWindow) to scan and manage system window handles safely using the __stdcall calling convention.

Fuzzy Window Matching: Utilized EnumWindows callback-driven title enumeration to support partial/fuzzy string matches (e.g., typing "Chrome" matches active tab titles) rather than requiring exact window title strings.

Asynchronous Off-Thread Scanning: Shifted native window search and focus calls to background daemon threads to prevent blocking the JavaFX Application Thread during render pulses, dispatching UI updates back via Platform.runLater.

Distinct Visual Feedback (BOUND State): Implemented a BOUND visual state (green color shift, 1.08× scale factor, and a visual indicator on the billboard label) to clearly signal when a node is linked to an active desktop window.

Cross-Platform Graceful Fallback: Wrapped native API calls in runtime availability checks (isAvailable()) so that non-Windows environments (macOS/Linux) or missing JNA runtimes fall back to standard non-bound node behavior without crashing.

Dependencies: Added net.java.dev.jna:jna-platform:5.14.0 dependency.
Claude's response:
What was added and why every decision was made the way it was
1. JNA interface — User32Extended
java
...
StdCallLibrary tells JNA to use the __stdcall calling convention that Win32 requires — without this the stack gets corrupted and the JVM crashes. W32APIOptions.DEFAULT_OPTIONS automatically selects the Unicode (W-suffix) variants like GetWindowTextW and handles char[] ↔ LPWSTR marshalling.

The three functions mapped:

Win32 function	What it does
EnumWindows	Calls a callback for every top-level window — lets us do partial matching
GetWindowTextW	Reads a window's title bar text into a char[] buffer
IsWindowVisible	Filters out invisible system windows (Task Manager internals, etc.)
SetForegroundWindow	Raises the window above all others and gives it focus
ShowWindow(SW_RESTORE)	Un-minimises if the window was minimised before focusing
2. Why EnumWindows instead of FindWindow

FindWindow requires an exact title or window class name. A user typing "chrome" would miss "New Tab — Google Chrome". EnumWindows fires our callback for every visible window so we can do title.contains(lowerCaseQuery) — the same fuzzy matching a user would expect from a launcher.

3. The one-element array trick in the callback
java
HWND[] result = { null };
...
Java lambdas can only close over effectively final variables. A single-element array is itself effectively final, but its contents are mutable — so result[0] is writable from inside the lambda. This is the idiomatic Java pattern for extracting a value from a callback.

4. Background threads for both scan and focus
java
Thread scanner = new Thread(() -> {
    HWND hwnd = WindowManager.findWindow(name);
    Platform.runLater(() -> { node.hwnd = hwnd; ... });
}, "winscan-" + name);
scanner.setDaemon(true);
scanner.start();

EnumWindows and SetForegroundWindow both make kernel transitions. Running them on the JavaFX Application Thread would block the 60-fps render pulse, causing a visible stutter. The scan runs off-thread; Platform.runLater marshals the result back to the FX thread for the safe UI mutation (node.hwnd = hwnd, applyVisualState, label.setText). Threads are marked daemon so they don't prevent JVM shutdown.

5. The BOUND visual state

A bound node gets a green tint (35% interpolation toward LIMEGREEN) and a 1.08× scale-up, plus a ⚡ appended to its billboard label. This gives three independent visual signals: color shift, size change, and text badge — so it's obvious even when nodes are far from the camera.

6. isAvailable() guard — graceful non-Windows degradation
java
...
Every JNA call site is wrapped in this check. On macOS or Linux, or if the JNA JARs aren't on the classpath, the app runs exactly as before — nodes just won't have window binding. This means the same JAR works on any OS.

7. Maven dependency to add
xml
<dependency>
...
jna-platform pulls in the base jna JAR transitively and already includes the HWND, BOOL, StdCallLibrary, and W32APIOptions types used here. No other dependency is needed.
#CODE#
...
public class WorkspaceGraph3D extends Application {
    ...
    private static class WindowManager {
         ...
         * {@code titleSubstring} (case-insensitive) and returns them as a
         * list ordered by EnumWindows enumeration order (roughly Z-order,
         * foreground window first).
         *
         * Returning ALL matches instead of just the first one lets the
         * caller show a disambiguation picker when there are multiple hits
         * (e.g. three Chrome windows each with a different tab as its title).
         */
            ...
            User32Extended.INSTANCE.EnumWindows((hwnd, lParam) -> {
                ...
                if (len > 0) {
                    ...
                    if (title.toLowerCase().contains(lower)) {
                        // Defensive copy: buf is reused each iteration,
                        // so we must capture the String before continuing.
                        results.add(new WindowEntry(hwnd, title));
                    }
                }
                return true;  // always continue — collect ALL matches
            }, null);
    ...
    /**
     * Spawns a disconnected node, then scans for matching OS windows.
     *
     * Resolution logic:
     *   0 matches → node stays unbound (no ⚡), spawns as a shortcut
     *   1 match   → auto-bind silently (same as before)
     *   2+ matches → open a picker dialog listing all full window titles
     *                so the user can pick the exact one they want
     */
    private void spawnNewNode() {
        ...
        // Scan on a background thread — EnumWindows blocks briefly
        Thread scanner = new Thread(() -> {
            List<WindowManager.WindowEntry> matches = WindowManager.findAllWindows(name);

            javafx.application.Platform.runLater(() -> {
                if (matches.isEmpty()) {
                    // No live window found — node stays as an unbound shortcut
                } else if (matches.size() == 1) {
                    // Exactly one match — bind silently, no picker needed
                    bindNode(node, matches.get(0));
                } else {
                    // Multiple matches — show picker so the user chooses
                    showWindowPicker(node, matches);
                }
            });
        }, "winscan-" + name);
        scanner.setDaemon(true);
    ...
    /**
     * Opens a lightweight JavaFX dialog listing every matching window title.
     * The user clicks a row to bind that specific HWND to the node.
     *
     * Built entirely from standard JavaFX controls — no Alert or Dialog
     * subclass needed.  The Stage is APPLICATION_MODAL so the user must
     * resolve the ambiguity before continuing (or close to leave unbound).
     */
    private void showWindowPicker(GraphNode node,
                                  List<WindowManager.WindowEntry> matches) {
        javafx.stage.Stage picker = new javafx.stage.Stage();
        picker.initModality(javafx.stage.Modality.APPLICATION_MODAL);
        picker.setTitle("Choose window for \"" + node.name + "\"");
        ...
        Label header = new Label(
            matches.size() + " windows match \"" + node.name + "\".\nClick one to bind the node to it.");
        header.setFont(Font.font("Monospace", 12));
        header.setTextFill(Color.web("#8B949E"));
        header.setPadding(new Insets(14, 16, 10, 16));
        header.setWrapText(true);
        root.getChildren().add(header);

        // ── One button per match ─────────────────────────────────────
        // Each button shows the full title bar text so the user can
        // distinguish "Gmail - Chrome" from "YouTube - Chrome" etc.
        for (WindowManager.WindowEntry entry : matches) {
        ...
        // ── Cancel row ───────────────────────────────────────────────
        Button cancel = new Button("Leave unbound");
        cancel.setMaxWidth(Double.MAX_VALUE);
        cancel.setFont(Font.font("Monospace", 11));
        cancel.setPadding(new Insets(10, 16, 10, 16));
        cancel.setStyle("""
            -fx-background-color: #0D1117;
            -fx-text-fill: #484F58;
            -fx-background-radius: 0;
        ...
        javafx.scene.Scene scene = new javafx.scene.Scene(root,
            Math.min(600, 480), -1);  // auto-height
