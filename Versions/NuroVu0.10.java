...
public class WorkspaceGraph3D extends Application {
    ...
    private static class WindowManager {
        ...
        static boolean isAvailable() {
            ...
            try {
                ...
                return false;  // JNA JARs not on classpath
            }
        }

        /**
         * Enumerates all visible top-level windows and returns the HWND of
         * the first one whose title contains {@code titleSubstring}
         * (case-insensitive).  Returns null if no match is found.
         *
         * EnumWindows calls our callback once per window on the current
         * thread.  We store the result in a one-element array so the
         * lambda can write to it (Java lambdas can only close over
         * effectively-final variables, but array contents are mutable).
         */
        static HWND findWindow(String titleSubstring) {
            ...
            User32Extended.INSTANCE.EnumWindows((hwnd, lParam) -> {
                ...
                if (len > 0) {
                    String title = new String(buf, 0, len).toLowerCase();
         ...
         *
         * ShowWindow(SW_RESTORE) first un-minimises the window if needed.
         * SetForegroundWindow then raises it above all other windows and
         * gives it keyboard focus.  Both calls are no-ops if hwnd is stale.
         */
     ...
     * After spawning, attempts to find a running OS window whose title
     * contains the node name (case-insensitive).  If found, the node is
     * bound to that window's HWND and gets the green BOUND visual state.
     */
    private void spawnNewNode() {
        ...
        // ── Try to bind to a live OS window ──────────────────────────
        // WindowManager.findWindow() calls EnumWindows on the calling
        // thread.  We run it on a background thread so it never stalls
        // the 60-fps JavaFX pulse, then marshal the result back to the
        // FX thread with Platform.runLater for the UI update.
            ...
            Thread scanner = new Thread(() -> {
                ...
                javafx.application.Platform.runLater(() -> {
                    if (hwnd != null) {
                        ...
                        // Append a ⚡ badge to the billboard label so the
                        // user can see at a glance that the node is live.
