...
public class WorkspaceGraph3D extends Application {
    ...
    private static final double NODE_RADIUS      = 20.0;   // slightly larger than before
    private static final int    SCENE_W          = 1100;
    private static final int    SCENE_H          = 700;

    // ── Starfield ──────────────────────────────────────────────────────
    private static final int    STAR_COUNT       = 420;
    private static final double STAR_SHELL_R     = 1400.0; // radius of the star sphere shell
    private static final double STAR_SIZE_MIN    = 0.7;
    private static final double STAR_SIZE_MAX    = 3.0;
    ...
    private static class WindowManager {
        ...
        interface User32Extended extends StdCallLibrary {
            ...
            // BOOL IsIconic(HWND hWnd) — true if the window is minimised
            boolean IsIconic(HWND hWnd);

            // BOOL GetWindowRect(HWND hWnd, RECT* lpRect)
            // Fills lpRect with the window's current screen coordinates.
            boolean GetWindowRect(HWND hWnd, int[] lpRect);

            // BOOL SetWindowPos(HWND, HWND, int x, int y, int cx, int cy, UINT flags)
            // Repositions and/or resizes a window.
        ...
        static final int SW_RESTORE  = 9;   // restore minimised window
        static final int SW_SHOW     = 5;   // show at current size/position

        // SetWindowPos flags
        static final int SWP_NOMOVE     = 0x0002;  // ignore X, Y params
        static final int SWP_NOACTIVATE = 0x0010;  // don't change Z-order
        static final int SWP_SHOWWINDOW = 0x0040;  // make visible
        ...
        /**
         * Brings a window to the foreground while preserving its exact
         * size and position.
         *
         * WHY the old SW_RESTORE caused resizing
         * ──────────────────────────────────────
         * SW_RESTORE means "if minimised, restore to pre-minimise state;
         * if maximised, restore to pre-maximise (normal) state".
         * For a window that is already in the normal (neither minimised nor
         * maximised) state, Windows still runs the restore path, which can
         * move or resize the window to whatever its "restored" rect happens
         * to be — not necessarily its current position and size.
         *
         * THE FIX
         * ───────
         * 1. Read the window's current pixel rect with GetWindowRect
         *    (left, top, right, bottom in screen coordinates).
         * 2. Check IsIconic to know if the window is minimised.
         *    - If minimised: call SW_RESTORE (the only way to un-minimise),
         *      then immediately call SetWindowPos with the saved dimensions
         *      to undo any resize that SW_RESTORE may have applied.
         *    - If not minimised: call SW_SHOW, which brings the window
         *      forward without touching its size or position at all.
         * 3. Call SetForegroundWindow to give it keyboard focus.
         *
         * GetWindowRect fills a 4-element int[] as {left, top, right, bottom}.
         * Width  = right  - left
         * Height = bottom - top
         * We pass SWP_NOMOVE so SetWindowPos only adjusts size, not position
         * (the window may have been moved by the user since it was minimised).
         */
        static void focusWindow(HWND hwnd) {
            ...
            // 1. Snapshot the current rect before we touch anything
            int[] rect = new int[4];   // { left, top, right, bottom }
            u.GetWindowRect(hwnd, rect);
            int width  = rect[2] - rect[0];
            int height = rect[3] - rect[1];

            // 2. Un-minimise or simply show, depending on current state
            if (u.IsIconic(hwnd)) {
                // Window is minimised — SW_RESTORE is required to show it,
                // then we pin back to the snapshotted size immediately.
                u.ShowWindow(hwnd, SW_RESTORE);
                u.SetWindowPos(hwnd, null,
                    0, 0, width, height,
                    SWP_NOMOVE | SWP_NOACTIVATE);
            } else {
                // Window is already visible — SW_SHOW brings it forward
                // without altering its size or position.
                u.ShowWindow(hwnd, SW_SHOW);
            }

            // 3. Raise to foreground and give keyboard focus
    ...
    private static class GraphNode {
        ...
        GraphNode(String name, Color baseColor, double x, double y, double z) {
            ...
            // Tight specular = sharp bright highlight (gem-like)
            material.setSpecularColor(Color.WHITE);
            material.setSpecularPower(48);
            // Subtle self-illumination so the node glows faintly with its
            // own colour even in shadowed areas — gives the "aura source" feel.
            material.setSelfIlluminationMap(null); // cleared by applyVisualState
            // We approximate self-illumination by slightly lightening diffuse:
            material.setDiffuseColor(baseColor.interpolate(Color.WHITE, 0.08));
    ...
    private static class GraphEdge {
        ...
        GraphEdge(GraphNode a, GraphNode b) {
            this.a = a; this.b = b;
            cylinder = new Cylinder(1.2, 1);
            PhongMaterial mat = new PhongMaterial();
            // Blend the two endpoint colours so each tether takes on the
            // hue of the nodes it connects — visually ties the graph together.
            Color blend = a.baseColor.interpolate(b.baseColor, 0.5);
            mat.setDiffuseColor(Color.color(
                Math.min(1, blend.getRed()   * 0.55 + 0.22),
                Math.min(1, blend.getGreen() * 0.55 + 0.22),
                Math.min(1, blend.getBlue()  * 0.55 + 0.32),
                0.60));
            mat.setSpecularColor(Color.color(0.8, 0.9, 1.0));
    ...
    // ── Starfield state ───────────────────────────────────────────────
    /** Each star: [sphere, phase] — phase drives the twinkle sine wave */
    private final List<Sphere> stars      = new ArrayList<>();
    private final List<Double> starPhases = new ArrayList<>();

    /** 2D Canvas layered on top of the SubScene for the aura glow effect */
    private javafx.scene.canvas.Canvas auraCanvas;
    ...
    private BorderPane buildUI() {
        ...
        // ── Lighting — richer than before ────────────────────────────
        // Slightly warmer ambient so nodes don't look flat in dark areas.
        AmbientLight ambient = new AmbientLight(Color.color(0.18, 0.18, 0.28));

        // Key light: cool blue-white from upper-left (main highlight)
        PointLight key = new PointLight(Color.color(0.85, 0.92, 1.0));
        key.setTranslateX(-250); key.setTranslateY(-350); key.setTranslateZ(-150);

        // Fill light: warm purple from lower-right (rim light feel)
        PointLight fill = new PointLight(Color.color(0.35, 0.15, 0.55, 0.8));
        fill.setTranslateX(300); fill.setTranslateY(200); fill.setTranslateZ(200);

        // Back light: faint teal from behind (separates nodes from background)
        PointLight back = new PointLight(Color.color(0.05, 0.35, 0.45, 0.5));
        back.setTranslateX(0); back.setTranslateY(0); back.setTranslateZ(800);

        world3D.getChildren().addAll(ambient, key, fill, back);

        // ── Starfield ────────────────────────────────────────────────
        // Stars are tiny Sphere objects placed on the surface of a large
        // imaginary sphere around the scene.  They rotate with world3D
        // so they drift naturally as the user spins the camera — giving
        // the impression of a real sky rather than a flat backdrop.
        // Each star gets a random phase offset for its twinkle sine wave.
        buildStarfield();

        // ── Aura canvas (2D overlay) ──────────────────────────────────
        // A transparent Canvas sits on top of the SubScene in the same
        // StackPane.  Every frame the AnimationTimer clears it and redraws
        // a soft radial gradient around each node's projected screen position.
        // Because it's a 2D Canvas drawn OVER the 3D SubScene, there is no
        // depth-sorting issue — we're just painting light halos on glass.
        auraCanvas = new javafx.scene.canvas.Canvas(SCENE_W - 230, SCENE_H);
        auraCanvas.setMouseTransparent(true);  // all mouse events pass through

        // ── Viewport ─────────────────────────────────────────────────
        // Deep space gradient background — radial dark navy → near-black
        StackPane viewport = new StackPane(subScene, auraCanvas);
        viewport.setStyle("""
            -fx-background-color:
        ...
        root.setStyle("-fx-background-color: #05080F;");
    ...
    /**
     * Populates world3D with STAR_COUNT tiny spheres scattered uniformly
     * on a sphere of radius STAR_SHELL_R using the Fibonacci lattice method.
     *
     * WHY Fibonacci lattice instead of pure random?
     * Random placement clusters — you get clumps and voids.  The Fibonacci
     * lattice gives near-uniform coverage with no clustering, so stars look
     * naturally spread without obvious gaps or piles.
     *
     * Each star gets:
     *   • A random grey shade between 0.55 and 1.0 (not pure white — more natural)
     *   • A random size between STAR_SIZE_MIN and STAR_SIZE_MAX
     *   • A random phase offset so twinkles are desynchronised
     */
    private void buildStarfield() {
        double goldenAngle = Math.PI * (3.0 - Math.sqrt(5.0)); // ~2.399 radians

        for (int i = 0; i < STAR_COUNT; i++) {
            // Fibonacci lattice point on unit sphere
            double y     = 1.0 - (i / (double)(STAR_COUNT - 1)) * 2.0; // -1 to +1
            double r     = Math.sqrt(Math.max(0, 1.0 - y * y));
            double theta = goldenAngle * i;
            double x     = Math.cos(theta) * r;
            double z     = Math.sin(theta) * r;

            double size  = STAR_SIZE_MIN + rng.nextDouble() * (STAR_SIZE_MAX - STAR_SIZE_MIN);
            double grey  = 0.55 + rng.nextDouble() * 0.45;

            Sphere star = new Sphere(size);
            PhongMaterial mat = new PhongMaterial();
            // Stars use a slightly warm-tinted grey — cooler stars are bluish,
            // warmer are yellowish; mixing both looks more realistic than pure grey.
            double warmth = rng.nextDouble();
            Color starColor = warmth > 0.7
                ? Color.color(grey, grey * 0.93, grey)          // slightly blue
                : warmth < 0.2
                    ? Color.color(grey, grey * 0.96, grey * 0.82) // slightly warm
                    : Color.color(grey, grey, grey);              // neutral
            mat.setDiffuseColor(starColor);
            mat.setSpecularColor(Color.WHITE);
            mat.setSpecularPower(2);
            star.setMaterial(mat);

            star.setTranslateX(x * STAR_SHELL_R);
            star.setTranslateY(y * STAR_SHELL_R);
            star.setTranslateZ(z * STAR_SHELL_R);

            stars.add(star);
            starPhases.add(rng.nextDouble() * Math.PI * 2); // random phase
            world3D.getChildren().add(star);
        }
    }
    ...
    // ══════════════════════════════════════════════════════════════════
    //  SCENE GRAPH SYNC  (called every AnimationTimer tick ~60fps)
    // ══════════════════════════════════════════════════════════════════

    private long   tickCount = 0;   // frame counter for twinkle phase

    private void syncSceneGraph() {
        tickCount++;
        double ay = rotateY.getAngle(), ax = rotateX.getAngle();

        // ── 1. Physics positions + billboard rotation ─────────────────
        for (GraphNode nd : nodes) {
            nd.applyPosition();
        ...
        // ── 2. Star twinkle ───────────────────────────────────────────
        // Each star's opacity oscillates with a sine wave at its own
        // frequency and phase so they all twinkle independently.
        // Using opacity rather than scale keeps the CPU cost near zero.
        double t = tickCount * 0.025;  // global time, advances ~1.5 rad/sec
        for (int i = 0; i < stars.size(); i++) {
            double phase = starPhases.get(i);
            // Two overlapping sine waves at different frequencies = natural shimmer
            double flicker = 0.55 + 0.45 * (
                0.6 * Math.sin(t * 1.1 + phase) +
                0.4 * Math.sin(t * 2.7 + phase * 1.3)
            );
            stars.get(i).setOpacity(Math.max(0.08, Math.min(1.0, flicker)));
        }

        // ── 3. Aura canvas — 2D glow rings drawn over the 3D scene ───
        // We project each node's 3D world position onto 2D screen space,
        // then paint a radial gradient centred on that screen point.
        //
        // PROJECTION MATH:
        // JavaFX's SubScene uses a standard perspective divide.
        // Given the camera at Z = -CAMERA_DISTANCE and a node at world
        // position (wx, wy, wz) after rotation, the screen position is:
        //
        //   viewZ  = world Z after applying rotateX and rotateY
        //   screenX = (wx / (wz + CAM_DIST)) * FOCAL + CANVAS_CX
        //
        // Instead of computing this ourselves, we query the Sphere node's
        // localToScene() bounds — JavaFX already did the full perspective
        // projection including all transforms.  The centre of those 2D
        // bounds IS the projected screen position, without any manual math.
        if (auraCanvas == null) return;
        javafx.scene.canvas.GraphicsContext gc = auraCanvas.getGraphicsContext2D();
        gc.clearRect(0, 0, auraCanvas.getWidth(), auraCanvas.getHeight());

        for (GraphNode nd : nodes) {
            try {
                javafx.geometry.Bounds b =
                    nd.sphere.localToScene(nd.sphere.getBoundsInLocal());
                // b is in Scene coordinates; subtract SubScene's layout offset.
                // Since SubScene is the first child of the StackPane and the
                // StackPane has no margin, the offset is (0,0) — b.getCenterX/Y
                // directly gives us canvas coordinates.
                double cx = b.getCenterX();
                double cy = b.getCenterY();
                double screenR = b.getWidth() / 2.0;  // projected sphere radius

                if (Double.isNaN(cx) || Double.isInfinite(cx)) continue;

                // Outer aura — large, very soft glow
                double outerR = screenR * 5.5;
                javafx.scene.paint.RadialGradient outerGlow =
                    new javafx.scene.paint.RadialGradient(
                        0, 0, cx, cy, outerR, false,
                        javafx.scene.paint.CycleMethod.NO_CYCLE,
                        new javafx.scene.paint.Stop(0.0,
                            Color.color(nd.baseColor.getRed(),
                                        nd.baseColor.getGreen(),
                                        nd.baseColor.getBlue(), 0.22)),
                        new javafx.scene.paint.Stop(0.45,
                            Color.color(nd.baseColor.getRed(),
                                        nd.baseColor.getGreen(),
                                        nd.baseColor.getBlue(), 0.07)),
                        new javafx.scene.paint.Stop(1.0, Color.TRANSPARENT)
                    );
                gc.setFill(outerGlow);
                gc.fillOval(cx - outerR, cy - outerR, outerR * 2, outerR * 2);

                // Inner corona — tight bright ring just outside the sphere
                double innerR = screenR * 2.2;
                javafx.scene.paint.RadialGradient innerGlow =
                    new javafx.scene.paint.RadialGradient(
                        0, 0, cx, cy, innerR, false,
                        javafx.scene.paint.CycleMethod.NO_CYCLE,
                        new javafx.scene.paint.Stop(0.0,  Color.TRANSPARENT),
                        new javafx.scene.paint.Stop(0.55,
                            Color.color(nd.baseColor.getRed(),
                                        nd.baseColor.getGreen(),
                                        nd.baseColor.getBlue(), 0.18)),
                        new javafx.scene.paint.Stop(0.80,
                            Color.color(nd.baseColor.getRed() * 0.7 + 0.3,
                                        nd.baseColor.getGreen() * 0.7 + 0.3,
                                        nd.baseColor.getBlue() * 0.7 + 0.3, 0.28)),
                        new javafx.scene.paint.Stop(1.0, Color.TRANSPARENT)
                    );
                ...
                // Bounds NaN on first frame before layout — safe to skip
            }
        }
    }
