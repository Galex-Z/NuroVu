...
public class WorkspaceGraph3D extends Application {
    ...
    // Yaw  (Y-axis): turns the camera left / right — like shaking your head "no"
    // Pitch(X-axis): tilts the camera up   / down  — like nodding your head "yes"
    //
    // Both transforms are applied TO THE CAMERA, not to world3D.
    // Order matters: yaw must wrap pitch so that pitching always feels
    // "up/down" relative to the current yaw, not the world Y-axis.
    // We achieve this by nesting: camera → yawGroup(rotateYaw) → pitchGroup(rotatePitch).
    private final Rotate rotateYaw   = new Rotate(0,  Rotate.Y_AXIS);
    private final Rotate rotatePitch = new Rotate(0,  Rotate.X_AXIS);
    private double cameraYaw   = -30.0;   // initial horizontal angle
    private double cameraPitch =  20.0;   // initial vertical angle
    private static final double PITCH_LIMIT = 89.0; // clamp: prevent gimbal flip
    ...
    private BorderPane buildUI() {
        ...
        // We wrap the camera in two nested Groups so yaw and pitch
        // compose correctly regardless of order:
        //
        //   pitchGroup  (rotatePitch around local X) ← innermost
        //       └── camera (translated back along Z)
        //   yawGroup    (rotateYaw   around world Y) ← outermost
        //       └── pitchGroup
        //
        // Reading transforms inward: first yaw rotates our horizontal
        // facing, then pitch tilts up/down within that facing direction.
        // This is exactly how FPS cameras work: yaw is absolute world-Y,
        // pitch is relative to the current yaw.
        camera.setNearClip(0.1);
        camera.setFarClip(4000);
        camera.setTranslateZ(-CAMERA_DISTANCE);

        Group pitchGroup = new Group(camera);
        pitchGroup.getTransforms().add(rotatePitch);

        Group yawGroup = new Group(pitchGroup);
        yawGroup.getTransforms().add(rotateYaw);

        // Apply stored initial angles
        rotateYaw.setAngle(cameraYaw);
        rotatePitch.setAngle(cameraPitch);

        // world3D has NO rotation transforms — it stays perfectly still.
        // Only the camera moves.
        subScene.setCamera(camera);
        world3D.getChildren().add(yawGroup);   // camera rig lives in world3D
    ...
    private void attachMouseHandlers(StackPane viewport) {
        ...
        viewport.setOnMouseDragged(e -> {
            ...
            if (e.isPrimaryButtonDown()) {
                // ── LEFT DRAG → FPS look-around ───────────────────────
                // Blocked in link mode so clicks on spheres don't spin
                if (linkModeActive) return;

                // Yaw: horizontal drag rotates left/right around world Y.
                // No clamping needed — full 360° is fine.
                cameraYaw += dx * ROTATION_SPEED;
                rotateYaw.setAngle(cameraYaw);

                // Pitch: vertical drag tilts up/down around local X.
                // Clamped to ±PITCH_LIMIT to prevent the camera flipping
                // upside-down past vertical (gimbal lock avoidance).
                cameraPitch += dy * ROTATION_SPEED;
                cameraPitch = Math.max(-PITCH_LIMIT, Math.min(PITCH_LIMIT, cameraPitch));
                rotatePitch.setAngle(cameraPitch);

            } else if (e.isSecondaryButtonDown()) {
                // ── RIGHT DRAG → pan camera position ─────────────────
                // We want dragging to slide the camera in the plane
                // perpendicular to where it's looking, so the pan always
                // feels "screen-aligned" regardless of yaw.
                //
                // We achieve this by translating world3D in the OPPOSITE
                // direction — same net visual effect, simpler math.
                // The pan is applied in world space so it doesn't need to
                // be decomposed into camera-local axes.
    ...
    private void syncSceneGraph() {
        // world3D no longer rotates — the camera does.
        // Billboards still need to face the camera, so we pass the current
        // yaw and pitch.  updateBillboard() applies the inverse of these
        // angles so the label always faces the viewer.
        for (GraphNode nd : nodes) {
            nd.applyPosition();
            nd.updateBillboard(cameraYaw, cameraPitch);
        }
