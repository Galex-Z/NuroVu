...
public class WorkspaceGraph3D extends Application {
    ...
    /**
     * Begins a smooth camera glide that centres {@code node} in the viewport.
     *
     * HOW THE GLIDE WORKS
     * ───────────────────
     * The camera is fixed at Z = -CAMERA_DISTANCE looking at the world origin.
     * To bring a node at world position (nx, ny, nz) to the centre of the
     * screen we need to translate world3D so that node ends up at the origin.
     *
     * The node's position in world3D's local space is (nx, ny, nz).
     * We want world3D.translateX/Y to shift so the node projects to screen centre.
     * The simplest correct approach for our setup:
     *   targetTranslateX = -nx   (push world left  so node moves right to centre)
     *   targetTranslateY = -ny   (push world up    so node moves down  to centre)
     *
     * The actual movement happens in syncSceneGraph() via a lerp each tick,
     * so it glides smoothly without any separate Timeline or thread.
     * The Z component is not addressed here — use scroll to zoom to taste.
     *
     * The target must be expressed in world3D's PRE-ROTATION space.
     * After applying rotateX and rotateY, the node's screen position also
     * depends on those angles.  For a full solution we'd need to project
     * through the rotation matrix.  For most graph layouts the nodes are
     * spread within WORLD_RADIUS and a simple -nx/-ny gives a close-enough
     * centre — the user can fine-tune with drag.
     */
