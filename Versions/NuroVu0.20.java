...
public class WorkspaceGraph3D extends Application {
    ...
    // When autoFocusEnabled is true, selecting a node triggers a smooth
    // pan glide that centres the node in the viewport.
    // The glide is driven by the existing AnimationTimer (no extra thread)
    // by interpolating world3D's translateX/Y toward target values each tick.
    private boolean autoFocusEnabled = false;
    private boolean glideActive      = false;
    private double  glideTargetX     = 0;   // target world3D.translateX
    private double  glideTargetY     = 0;   // target world3D.translateY
    private static final double GLIDE_SPEED = 0.10; // lerp factor per tick (0-1)
    ...
    private javafx.scene.control.ScrollPane buildSidebar() {
        ...
        sidebar.getChildren().addAll(
            ...
            divFocus, focusTitle, autoFocusBtn, focusHint
        );
    ...
    private void selectNode(GraphNode node) {
        ...
        // If auto-focus is on, start a smooth glide toward this node
        if (autoFocusEnabled) startGlideTo(node);
    }

    /**
     * Begins a smooth glide that centres {@code node} in the viewport,
     * correctly accounting for the current rotation of world3D.
     *
     * THE PROBLEM WITH THE OLD APPROACH
     * ──────────────────────────────────
     * The old version set glideTargetX = -node.x, glideTargetY = -node.y.
     * That works only when the world has zero rotation, because world3D's
     * translateX/Y operates in the PARENT (screen) coordinate space, while
     * node.x/y are in world3D's LOCAL coordinate space.  After a rotation
     * those two spaces are misaligned, so the glide landed in the wrong place.
     *
     * THE CORRECT APPROACH — project through the scene graph
     * ───────────────────────────────────────────────────────
     * JavaFX already knows exactly where the sphere is on screen after
     * applying every transform (rotation, pan, perspective).  We ask it:
     *
     *   bounds = sphere.localToScene(sphere.getBoundsInLocal())
     *
     * This gives us the sphere's 2D screen-space centre (cx, cy).
     * The viewport centre is (vpW/2, vpH/2).
     * The offset that would move the sphere to screen centre is:
     *
     *   deltaScreenX = vpCX - cx
     *   deltaScreenY = vpCY - cy
     *
     * world3D.translateX/Y IS in screen space (parent of world3D is the
     * SubScene root, which is screen-aligned), so we can add these deltas
     * directly to the current translation:
     *
     *   glideTargetX = world3D.getTranslateX() + deltaScreenX
     *   glideTargetY = world3D.getTranslateY() + deltaScreenY
     *
     * This is rotation-invariant: it doesn't matter what rotateX/rotateY
     * are set to — the projection already baked them in.
     */
    private void startGlideTo(GraphNode node) {
        try {
            javafx.geometry.Bounds b =
                node.sphere.localToScene(node.sphere.getBoundsInLocal());

            if (Double.isNaN(b.getCenterX())) return; // not yet laid out

            // Viewport dimensions (SubScene width = total - sidebar)
            double vpW = SCENE_W - 230;
            double vpH = SCENE_H;
            double vpCX = vpW / 2.0;
            double vpCY = vpH / 2.0;

            // How far the sphere's screen centre is from the viewport centre
            double deltaX = vpCX - b.getCenterX();
            double deltaY = vpCY - b.getCenterY();

            // Add that delta to world3D's current translation
            glideTargetX = world3D.getTranslateX() + deltaX;
            glideTargetY = world3D.getTranslateY() + deltaY;
            glideActive  = true;

        } catch (Exception ignored) {
            // Scene graph not ready — skip the glide gracefully
        }
    }
    ...
    private void attachMouseHandlers(StackPane viewport) {
        ...
        viewport.setOnMouseDragged(e -> {
            // Any manual drag immediately cancels an in-progress glide
            // so the user always gets instant manual control back.
            glideActive = false;
    ...
    private void syncSceneGraph() {
        ...
        if (glideActive) {
            ...
            if (Math.abs(glideTargetX - newX) < 0.5 &&
                Math.abs(glideTargetY - newY) < 0.5) {
