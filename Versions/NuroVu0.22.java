...
public class WorkspaceGraph3D extends Application {
    ...
    private static final double FOCUS_DISTANCE   = 250.0; // camera Z when auto-focusing a node
    ...
    private double  glideTargetZ     = -FOCUS_DISTANCE; // target camera.translateZ
     ...
     * and centres {@code node} in the viewport.
     *
     * WHY RESETTING ANGLES MAKES THE MATH TRIVIAL
     * ────────────────────────────────────────────
     * When rotateX = 0 and rotateY = 0, world3D's local X/Y axes are
     * perfectly aligned with screen X/Y.  At that point, translating
     * world3D by (-node.x, -node.y) moves the node exactly to the screen
     * centre — no matrix decomposition, no localToScene() projection.
     *
     * We animate all four values simultaneously so the rotation unwinds
     * and the pan converges in one smooth motion.
     */
    private void startGlideTo(GraphNode node) {
        glideTargetX  = -node.x;
        glideTargetY  = -node.y;
        glideTargetZ  = -FOCUS_DISTANCE;
        glideTargetRX = 0;
        glideTargetRY = 0;
        glideActive   = true;
    }
