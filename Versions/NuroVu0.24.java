...
public class WorkspaceGraph3D extends Application {
    ...
    private double  glideTargetZ     = -CAMERA_DISTANCE;
     ...
     * a comfortable padding multiplier.  That gives a spread radius.
     * The camera Z target is then:
     *
     *   targetZ = -(node.z + spread * FOCUS_PADDING + NODE_RADIUS * 3)
     *
     ...
     */
    private static final double FOCUS_PADDING   = 2.2;  // how much breathing room around neighbours
    private static final double FOCUS_FALLBACK  = 380.0; // distance when node has no edges
    ...
    private void startGlideTo(GraphNode node) {
        ...
        // Compute the spread of connected neighbours
        double maxNeighbourDist = 0;
        for (GraphEdge e : edges) {
            GraphNode neighbour = null;
            if (e.a == node) neighbour = e.b;
            else if (e.b == node) neighbour = e.a;
            if (neighbour != null) {
                double dx = neighbour.x - node.x;
                double dy = neighbour.y - node.y;
                double dz = neighbour.z - node.z;
                double d  = Math.sqrt(dx*dx + dy*dy + dz*dz);
                if (d > maxNeighbourDist) maxNeighbourDist = d;
            }
        }

        double spread   = maxNeighbourDist > 0 ? maxNeighbourDist : FOCUS_FALLBACK;
        double targetZ  = -(node.z + spread * FOCUS_PADDING + NODE_RADIUS * 3);
        glideTargetZ    = Math.min(-ZOOM_MIN, targetZ); // never closer than near clip
    ...
    private void syncSceneGraph() {
        ...
        if (glideActive) {
            ...
            double curZ  = camera.getTranslateZ();
            ...
            double newZ  = curZ  + (glideTargetZ  - curZ)  * GLIDE_SPEED;
            ...
            if (Math.abs(glideTargetX  - newX)  < 0.4  &&
                ...
                Math.abs(glideTargetZ  - newZ)  < 0.4  &&
                ...
                camera.setTranslateZ(glideTargetZ);
                rotateX.setAngle(0);
                rotateY.setAngle(0);
                glideActive = false;
            }
