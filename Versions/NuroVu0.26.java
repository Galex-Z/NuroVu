...
public class WorkspaceGraph3D extends Application {
    ...
    private void saveGraph() {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            // boundTitle: the window title we matched against, or ""
            // We stored the title in the label text with a ⚡ suffix —
            // strip it to recover the clean name for re-matching on import.
            String boundTitle = "";
            if (n.isBound()) {
                // Re-scan to find the current title for this HWND
                for (WindowManager.WindowEntry we : WindowManager.listAllWindows()) {
                    if (we.hwnd().equals(n.hwnd)) { boundTitle = we.title(); break; }
                }
            }
    ...
    private void loadGraph() {
        ...
        try {
            ...
            for (String obj : nodeObjs) {
                ...
                // Rebind: find the first open window whose title contains
                // the saved boundTitle (case-insensitive)
                if (!boundTitle.isEmpty()) {
                    String lower = boundTitle.toLowerCase();
                    for (WindowManager.WindowEntry we : openWindows) {
                        if (we.title().toLowerCase().contains(lower)) {
                            node.hwnd = we.hwnd();
                            node.applyVisualState(GraphNode.VisualState.BOUND);
    ...
    /**
     * Extract the text between the first occurrence of {@code startKey}
     * and the next occurrence of {@code endToken} after it.
     * Used to pull out the nodes and edges arrays.
     */
    private static String between(String json, String startKey, String endToken) {
        ...
        s = json.indexOf("[", s);
        if (s < 0) return "";
        int e = json.lastIndexOf(endToken);
        if (e <= s) return "";
