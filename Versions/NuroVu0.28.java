...
public class WorkspaceGraph3D extends Application {
    ...
    private static class GraphNode {
        ...
        // OS window binding
        HWND   hwnd       = null;
        String boundTitle = "";   // stored at bind time, used for save/load
    ...
    private void bindNode(GraphNode node, WindowManager.WindowEntry entry) {
        ...
        node.boundTitle = entry.title();   // stored here, read directly on save
    ...
    private void saveGraph() {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            // boundTitle is stored on the node at bind time — read directly
            String boundTitle = n.boundTitle;
            ...
            sb.append("    { \"id\": ").append(i)
              ...
              .append(", \"boundTitle\": ").append(jsonStr(boundTitle))
              .append(" }");
    ...
    private void loadGraph() {
        ...
        try {
            ...
            // Pre-fetch all open windows once for rebinding
            List<WindowManager.WindowEntry> openWindows =
                WindowManager.isAvailable() ? WindowManager.listAllWindows() : List.of();
            ...
            for (String obj : nodeObjs) {
                ...
                String boundTitle = strVal(obj, "boundTitle");

                GraphNode node = addNodeAt(name, Color.web(color), x, y, z);
                // Spawn damping: loaded nodes are already placed, skip the
                // launch animation by marking them as fully settled.
                node.spawnTick    = SPAWN_DAMP_TICKS;
                node.spawnDamping = DAMPING;
                loaded.add(node);

                // Rebind: match the saved boundTitle against open windows.
                //
                // Window titles change between sessions (e.g. the active file
                // shown in VS Code's title bar changes).  We try three levels
                // of matching in order, most-specific to least:
                //
                //   1. Exact match (case-insensitive)
                //   2. Saved title contains the current window title
                //      (saved was more specific — current is a shorter base)
                //   3. Current window title contains the saved title
                //      (current has extra context added — saved was the base)
                //
                // This way "Visual Studio Code" matches both
                // "Visual Studio Code - file.java" (rule 3) and
                // "file.java - Visual Studio Code" (rule 3).
                if (!boundTitle.isEmpty()) {
                    String savedLower = boundTitle.toLowerCase();
                    WindowManager.WindowEntry best = null;
                    int bestScore = -1;

                    for (WindowManager.WindowEntry we : openWindows) {
                        String liveLower = we.title().toLowerCase();
                        int score;
                        if (liveLower.equals(savedLower))             score = 3; // exact
                        else if (savedLower.contains(liveLower))      score = 2; // live is subset of saved
                        else if (liveLower.contains(savedLower))      score = 1; // saved is subset of live
                        else                                          score = -1;

                        if (score > bestScore) { bestScore = score; best = we; }
                    }

                    if (best != null && bestScore >= 1) {
                        node.hwnd       = best.hwnd();
                        node.boundTitle = best.title();
                        node.applyVisualState(GraphNode.VisualState.BOUND);
