...
public class WorkspaceGraph3D extends Application {
    ...
    public void start(Stage stage) {
        ...
        // When fullscreen state changes (entering OR exiting), force a
        // layout refresh and bring the 2D control panel to the front so
        // it is never occluded by the 3D SubScene container.
        stage.fullScreenProperty().addListener((obs, wasFullScreen, isFullScreen) -> {
            if (!isFullScreen) {
                stage.setWidth(SCENE_W);
                stage.setHeight(SCENE_H);
            }
            // Give JavaFX one pulse to settle the new window dimensions,
            // then requestLayout + toFront so the sidebar is always on top.
            javafx.application.Platform.runLater(() -> {
                rootLayout.requestLayout();
                sidebarPanel.toFront();
            });
        });
    ...
    private javafx.scene.control.ScrollPane buildSidebar() {
        ...
        scroll.setMinWidth(238);   // never collapses when window shrinks back from fullscreen
        scroll.setMaxWidth(238);   // never grows wider than the fixed sidebar width
