...
public class WorkspaceGraph3D extends Application {
    ...
    private BorderPane                      rootLayout;      // top-level layout node
    private javafx.scene.control.ScrollPane sidebarPanel;   // the 2D control panel
    ...
    public void start(Stage stage) {
        ...
        // UNDECORATED removes the OS title bar entirely — set before show().
        // We draw our own thin title bar inside buildUI() so the user can
        // still drag and close the window.  This gives us full control of
        // the layout at all times: maximized or windowed, the BorderPane
        // always fills the stage correctly with no OS-chrome interference.
        ...
        scene.setOnKeyPressed(e -> {
            switch (e.getCode()) {
                ...
                case F11 -> stage.setMaximized(!stage.isMaximized());
                default -> {}
            }
        });
    ...
    private BorderPane buildUI(Stage stage) {
        // ── Custom title bar ─────────────────────────────────────────
        // Replaces the OS title bar removed by UNDECORATED.
        // Dragging it moves the window; double-clicking toggles maximized.
        Label titleLabel = new Label("  3D Workspace Graph   |   F11 to maximise");
        titleLabel.setFont(Font.font("Monospace", 11));
        titleLabel.setTextFill(Color.web("#8B949E"));
        ...
        closeBtn.setOnMouseEntered(e ->
            closeBtn.setStyle(closeBtn.getStyle().replace("#8B949E", "#FF5F57")));
        closeBtn.setOnMouseExited(e ->
            closeBtn.setStyle(closeBtn.getStyle().replace("#FF5F57", "#8B949E")));
        closeBtn.setOnAction(e -> stage.close());

        HBox titleBar = new HBox(titleLabel, closeBtn);
        titleBar.setStyle("-fx-background-color: #0D1117; -fx-border-color: #21262D; -fx-border-width: 0 0 1 0;");
        ...
        // Bind SubScene to viewport so it fills all available space,
        // including during maximize and manual resize.
        ...
        // ── Main content (viewport + sidebar side by side) ───────────
        sidebarPanel = buildSidebar();
        HBox content = new HBox(viewport, sidebarPanel);
        HBox.setHgrow(viewport, javafx.scene.layout.Priority.ALWAYS);

        // ── Root: title bar on top, content below ────────────────────
