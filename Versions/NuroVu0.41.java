...
public class WorkspaceGraph3D extends Application {
    ...
    private Stage buildAssistantStage() {
        ...
        // ── Assistant SubScene using assistWorld ──────────────────────
        // assistWorld is a separate Group (different root from world3D)
        // so JavaFX's single-parent constraint is satisfied.
        // It shares the same Rotate transforms as world3D (rotation is
        // auto-synced), and its translateX/Y is bound to world3D (pan synced).
        // assistCam.translateZ is bound to the main camera (zoom synced).
        int   AW = 240, AH = 370;
        ...
        // ── Aspect ratio fix ──────────────────────────────────────────
        // The assistant window is narrow (240px wide).  Without correction,
        // JavaFX would use the SubScene's own width/height ratio for the
        // perspective projection, making spheres appear as tall ovals.
        // We set the camera's fieldOfView and a horizontal viewport width
        // that matches the MAIN window's aspect ratio so spheres stay round.
        // The trick: use a wider virtual FOV that compensates for the narrow
        // physical canvas by scaling the horizontal projection to match the
        // main view's aspect ratio (SCENE_W - 238) / SCENE_H.
        double mainAspect  = (double)(SCENE_W - 238) / SCENE_H;   // ~1.24
        double assistAspect = (double) AW / AH;                    // ~0.65
        // Scale the horizontal FOV so the projected width matches the main aspect.
        // Effective horizontal FOV = 2 * atan( tan(vFOV/2) * aspect )
        // We solve for a vFOV that gives the same horizontal span as main.
        double vFovMain   = Math.toRadians(45.0);
        double hFovMain   = 2 * Math.atan(Math.tan(vFovMain / 2) * mainAspect);
        double vFovAssist = 2 * Math.atan(Math.tan(hFovMain / 2) / assistAspect);
        assistCam.setFieldOfView(Math.toDegrees(vFovAssist));
        ...
        javafx.scene.Scene scene = new javafx.scene.Scene(root, AW, AH + 40,
                                                           Color.web("#0D1117"));
        stage.setScene(scene);
