...
public class WorkspaceGraph3D extends Application {
    ...
    /** Remove all proxy nodes and edges from assistWorld, keeping lights. */
    private void clearAssistWorld() {
        assistWorld.getChildren().removeIf(child -> {
            // Keep lights — remove spheres, billboards, cylinders
            return !(child instanceof AmbientLight) && !(child instanceof PointLight);
        });
    }

    /** Re-add all current proxy nodes and edges to assistWorld after a reopen. */
    private void repopulateAssistWorld() {
        clearAssistWorld(); // ensure no duplicates
