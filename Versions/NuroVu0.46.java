...
public class WorkspaceGraph3D extends Application {
    ...
    private       Group             assistWorld = new Group();
    ...
    private BorderPane buildUI() {
        ...
        // assistWorld is set up fresh each time the assistant opens
        // via repopulateAssistWorld() — no setup needed here.
    ...
    private javafx.scene.control.ScrollPane buildSidebar() {
        ...
        assistantBtn.setOnAction(e -> {
            if (assistantStage == null || !assistantStage.isShowing()) {
                // Repopulate assistWorld with current proxy nodes/edges
                // in case it was cleared on a previous close.
                repopulateAssistWorld();
                ...
                assistantStage = null;
                // Clear assistWorld so proxies can be re-added cleanly next open
                clearAssistWorld();
    ...
    private Stage buildAssistantStage() {
        ...
        // Fresh camera every open — JavaFX holds a hard reference from
        // SubScene to Camera just like it does for the root Group, so
        // reusing the same Camera instance across reopens throws the same
        // "already set as camera in other scene" error.
        PerspectiveCamera assistCam = new PerspectiveCamera(true);
    ...
    /** Rebuild assistWorld as a fresh Group so it has no prior SubScene owner. */
    private void repopulateAssistWorld() {
        // A Group can only be root of one SubScene at a time and JavaFX
        // never releases that reference even after the Stage closes.
        // Solution: create a new Group each open so it has no prior owner.
        assistWorld = new Group();

        // Shared rotation transforms — same objects as world3D
        assistWorld.getTransforms().addAll(rotateX, rotateY);

        // Bind pan to world3D so right-drag mirrors
        ...
        // Add current proxy nodes and edges
        for (GraphNode nd : nodes)
            assistWorld.getChildren().addAll(nd.proxySphere, nd.proxyBillboard);
        for (GraphEdge e : edges)
            assistWorld.getChildren().add(e.proxyCylinder);
    }

    /** No-op kept for symmetry — assistWorld is discarded on close, not cleared. */
    private void clearAssistWorld() {
        // Nothing to do — the old assistWorld Group is simply abandoned
        // and garbage collected. The new one is built fresh on next open.
    }
